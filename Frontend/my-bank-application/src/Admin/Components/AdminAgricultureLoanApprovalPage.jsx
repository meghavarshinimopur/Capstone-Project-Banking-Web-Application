...
const AdminAgricultureLoanApprovalPage = () => {
  ...
  // Fetch pending agriculture loans on component mount
  useEffect(() => {
    const fetchPendingLoans = async () => {
      try {
        const adminUsername = JSON.parse(localStorage.getItem("admin")).username; // Retrieve admin username
        setIsLoading(true);
        const response = await axios.get("http://localhost:8080/agricultureLoan/pending", {
  ...
  const confirmApproveLoan = async () => {
    try {
      const adminUsername = JSON.parse(localStorage.getItem("admin")).username; // Retrieve admin username
      setIsLoading(true);
      await axios.put(`http://localhost:8080/admin/approve/agricultureLoan/${selectedLoanId}`, null, {
        headers: { "Admin-Username": adminUsername },
      });
  ...
  return (
    <div className="d-flex flex-column min-vh-100">
      ...
      <div className="container flex-grow-1">
        <h1 className="text-center my-4">Admin: Approve Agriculture Loan Applications</h1>

        {/* Pending Loans Table */}
        <div className="loan-table bg-light p-4 rounded shadow-sm">
          <h5>Pending Agriculture Loan Applications</h5>
...
export default AdminAgricultureLoanApprovalPage;
